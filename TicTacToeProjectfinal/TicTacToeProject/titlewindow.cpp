...
#include <QPixmap>
#include <QDebug>
#include <QDialog>      // Include QDialog for creating dialog boxes
      ...
      , ui(new Ui::TitleWindow)
{
  ...
  QPixmap pixmap("C:/Users/pc/Documents/kkkkkkkkkkkkkk/TicTacToeGame-main/TicTacToeGame-main/TicTacToeProjectfinal/TicTacToeProject/images/icon.png");
  if (pixmap.isNull()) {
  ...
  connect(ui->registerbutton, SIGNAL(clicked()), this, SLOT(registering()));
...
void TitleWindow::registering()
{
  // Create a dialog for LoginForm
  QDialog *loginDialog = new QDialog(this);
  loginDialog->setWindowTitle("Login");
  loginDialog->resize(400, 300); // Adjust the size here
  LoginForm *loginForm = new LoginForm(loginDialog);
  QVBoxLayout *loginLayout = new QVBoxLayout(loginDialog);
  loginLayout->addWidget(loginForm);
  loginDialog->setLayout(loginLayout);
  loginDialog->exec();

         // Create a dialog for Form
  QDialog *formDialog = new QDialog(this);
  formDialog->setWindowTitle("Form");
  formDialog->resize(400, 300); // Adjust the size here
  Form *form = new Form(formDialog);
