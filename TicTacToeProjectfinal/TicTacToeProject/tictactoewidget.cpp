...
TicTacToeWidget::TicTacToeWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::TicTacToeWidget) {
  ui->setupUi(this);
  ...
  connect(this, SIGNAL(startaimovecalculation()), this,
          SLOT(calculateaimove()));
}
 ...
 */
Players TicTacToeWidget::getplayer() const { return player; }
...
void TicTacToeWidget::createBoard() {
  ...
  for (int row = 0; row < MetaData::ROWS; row++) {
    for (int column = 0; column < MetaData::COLUMNS; column++) {
      ...
      // The map signal emitted is then connected to a slot handleclicksonboard
      // and the index is passed to it.
 ...
 */
Winner TicTacToeWidget::determinewinner(const QString &symbol,
                                        int buttonindex) {
...
int TicTacToeWidget::horizontalcheck(const QString &symbol, int rownumber,
                                     int columnnumber) {
...
int TicTacToeWidget::verticalcheck(const QString &symbol, int rownumber,
                                   int columnnumber) {
  int counter = 1;  // Start from 1 to count the current position
...
int TicTacToeWidget::backwardslashdiagonalcheck(const QString &symbol,
                                                int rownumber,
                                                int columnnumber) {
  int counter = 1;  // Start from 1 to count the current position
  int newrow = rownumber - 1;
...
int TicTacToeWidget::forwardslashdiagonalcheck(const QString &symbol,
                                               int rownumber,
                                               int columnnumber) {
  int counter = 1;  // Start from 1 to count the current position
  int newrow = rownumber + 1;
...
TicTacToeWidget::~TicTacToeWidget() { delete ui; }
/**
...
void TicTacToeWidget::handleclicksonboard(int buttonindex) {
  ...
  if (player == Players::Player1) {
    ...
    // MetaData::player1colour +";background: lightgreen;}");
    // button->setDisabled(true);
  } else if (player == Players::Player2) {
    symbol = "O";
    button->setText(symbol);
    // button->setStyleSheet(QString("QPushButton{color:") +
    // MetaData::player2colour +";background: lightyellow}");
    // button->setDisabled(true);
  }
...
void TicTacToeWidget::triggeraimodecalculation() {
  if (player == Players::Player2) {
    this->setDisabled(true);
    QTimer::singleShot(MetaData::aidelay, this,
 ...
 **
    ...
    //ensure a free spot
    while(randomnumber>= MetaData::ROWS*MetaData::COLUMNS ||
 player1Moves.contains(randomnumber)
...
void TicTacToeWidget::calculateaimove() {
  ...
  int bestValue =
      std::numeric_limits<int>::min();  // Initialize the best value as the
  ...
  for (int i = 0; i < MetaData::BOARDSIZE; i++) {
    ...
    if (board.at(i)->text() == MetaData::space) {
      ...
      int moveValue = minimax(0, false, std::numeric_limits<int>::min(),
...
int TicTacToeWidget::minimax(int depth, bool isMax, int alpha, int beta) {
  ...
  if (isMax) {
    ...
    for (int i = 0; i < MetaData::BOARDSIZE; i++) {
      ...
      if (board.at(i)->text() == MetaData::space) {
        ...
        // Alpha-beta pruning: if beta is less than or equal to alpha, break the
        // loop
        if (beta <= alpha) break;
      }
    }
  ...
  // If it's the minimizer's turn (Player's turn)
    ...
    int best = std::numeric_limits<int>::max();  // Initialize the best value as
    ...
    for (int i = 0; i < MetaData::BOARDSIZE; i++) {
      ...
      if (board.at(i)->text() == MetaData::space) {
        board.at(i)->setText(
            MetaData::Player1);  // Make the move (Player uses 'X')
...
int TicTacToeWidget::evaluate() {
  ...
  for (int row = 0; row < MetaData::ROWS; row++) {
    if (board.at(row * MetaData::COLUMNS)->text() ==
            board.at(row * MetaData::COLUMNS + 1)->text() &&
        board.at(row * MetaData::COLUMNS + 1)->text() ==
            board.at(row * MetaData::COLUMNS + 2)->text()) {
      if (board.at(row * MetaData::COLUMNS)->text() == MetaData::AI)
        return 10;
      else if (board.at(row * MetaData::COLUMNS)->text() == MetaData::Player1)
        return -10;
    }
  }
  ...
  for (int col = 0; col < MetaData::COLUMNS; col++) {
    if (board.at(col)->text() == board.at(MetaData::COLUMNS + col)->text() &&
        board.at(MetaData::COLUMNS + col)->text() ==
            board.at(2 * MetaData::COLUMNS + col)->text()) {
      if (board.at(col)->text() == MetaData::AI)
        return 10;
      else if (board.at(col)->text() == MetaData::Player1)
        return -10;
    }
  }

  // Check diagonals for victory
  if (board.at(0)->text() == board.at(4)->text() &&
      board.at(4)->text() == board.at(8)->text()) {
    if (board.at(0)->text() == MetaData::AI)
      return 10;
    else if (board.at(0)->text() == MetaData::Player1)
      return -10;
  }
  if (board.at(2)->text() == board.at(4)->text() &&
      board.at(4)->text() == board.at(6)->text()) {
    if (board.at(2)->text() == MetaData::AI)
      return 10;
    else if (board.at(2)->text() == MetaData::Player1)
      return -10;
  }
 ...
 */
void TicTacToeWidget::setaimode() { mode = Modedata::Aimode; }

void TicTacToeWidget::setTwoplayermode() { mode = Modedata::Twoplayermode; }
